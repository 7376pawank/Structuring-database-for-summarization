...
def json_data_func(judgement):
    ...
    if(len(judgename.split(" ")) >= 5):
        # if re.match(r'^(JJ\.\] |J\.\] )', judgename):
        #     judgename=re.sub(r'\bJ\.]\s?\[?|\bJJ\.]\s?\[?', '', judgename)
        if re.match(r'\[(.*?)\]', judgename):
            judgename = re.findall(r'\[(.*?)\]', judgename)
        else:
            words = judgename.split()
            word_counts = Counter(words)
            repeated_words = [word for word, count in word_counts.items() if count == 2]
            judgename = " ".join(repeated_words)
    print(judgename)
